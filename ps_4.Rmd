...
z <- data %>% 
  ...
  mutate(likely = fct_relevel(likely, c( "Not at all likely","Not very likely",
                                        "Somewhat likely", "Very likely",
                                        "Almost certain", "Already voted"))) %>% 
#The likely variable is adjusted to exclude cases where it is unknown and it is releveled so that the variable can later be applied in the graph meaningfully and having a logical order.
  group_by(partyid, response, likely) %>% 
#This grouping explores the combination of the three variables of interest.
  select(partyid, response, likely, final_weight) %>% 
...
z %>% 
  ggplot(aes(x= response, y = total, fill = likely))+
